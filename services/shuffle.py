...
def create_shuffled_playlist(spotify_access_info, playlist_id, playlist_name):
    ...
    shuffled_playlist = spotify.user_playlist_create(user=user_id, name=SHUFFLED_PLAYLIST_PREFIX + playlist_name, public=False, description="Shuffled by True Shuffle")
    ...
    for i in range(calls_required):
        if i == calls_required - 1:
            add_items_response = spotify.playlist_add_items(shuffled_playlist["id"], all_tracks[i*100: i*100+left_over])
        else:
            add_items_response = spotify.playlist_add_items(shuffled_playlist["id"], all_tracks[i*100: i*100+100])
